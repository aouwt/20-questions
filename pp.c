...
typedef struct {
 char name [20 +2];
 unsigned char q[((sizeof(Questions) / sizeof(Questions[0])) - 1)+1];
} tdat;
typedef struct {
 tdat info;
 float chance;
} character;
character *Characters; unsigned int CharactersLen = 0;
tdat *TrainingDat; unsigned int TrainingDatLen = 0;
unsigned char CurAns [((sizeof(Questions) / sizeof(Questions[0])) - 1)+1];
unsigned int Target = 0;
float chance (character *C) {
 unsigned int e = 0;
 for (unsigned int i = 0; i != ((sizeof(Questions) / sizeof(Questions[0])) - 1); i++) {
  if ((CurAns [i] == 0) || (C -> info.q [i] == 0)) continue;
  if (CurAns [i] == C -> info.q [i])
   e++;
  else
   e = ((int)e - (int)(((sizeof(Questions) / sizeof(Questions[0])) - 1)/2) < 0) ? 0 : e - (((sizeof(Questions) / sizeof(Questions[0])) - 1)/2);
 }
 return (C -> chance = (e / ((sizeof(Questions) / sizeof(Questions[0])) - 1)));
}
unsigned char parseans (char a) {
 switch (a) {
...
unsigned int highestchance (void) {
 float curch = 0;
 unsigned int h = 0;
 for (unsigned int c = 0; c != TrainingDatLen; c++) {
...
void train (unsigned int qu, unsigned char an) {
 Target = highestchance ();
 CurAns [qu] = an;
}
unsigned char loadchars (void) {
 {
  char answers [((sizeof(Questions) / sizeof(Questions[0])) - 1) + 2];
  FILE* f = fopen (".td", "r");
  if (f == NULL) return 1;
  unsigned int ent;
  for (ent = 0;; ent++) {
   {
    char fmt [20];
    snprintf (fmt, (sizeof(fmt) / sizeof(fmt[0])), "%%%u[^,],%%%u[^\n]\n", (unsigned int)(20 +1), (unsigned int)((sizeof(Questions) / sizeof(Questions[0])) - 1)+1);
 ...
 for (unsigned int ent = 0; ent != TrainingDatLen; ent++) {
...
unsigned char savechars (void) {
 FILE* f = fopen (".td", "w");
 if (f == NULL) return 1;
 for (unsigned int ent = 0; ent != CharactersLen; ent++) {
  char answers [((sizeof(Questions) / sizeof(Questions[0])) - 1) + 2];
  for (unsigned int qu = 0; qu != ((sizeof(Questions) / sizeof(Questions[0])) - 1)+1; qu++) {
   switch (Characters[ent].info.q[qu]) {
    case 2: answers[qu] = 'T'; break;
    case 1: answers[qu] = 'F'; break;
    case 0: answers[qu] = '?'; break;
   }
  }
  answers [((sizeof(Questions) / sizeof(Questions[0])) - 1) + 1] = '\0';
  fprintf (f, "%s,%s\n", Characters[ent].info.name, answers);
 }
 fclose (f);
 return 0;
}
unsigned int getquestion (void) {
 unsigned int q;
 for (;;) {
  while (CurAns[(q = rand() % (((sizeof(Questions) / sizeof(Questions[0])) - 1)))] != 0);
  if ((rand () % 2) == 0) {
   if (Characters[Target].info.q[q] == 0) return q;
  } else return q;
 }
}
void init (void) {
...
void trainchar (unsigned int c) {
 for (unsigned int q = 0; q != ((sizeof(Questions) / sizeof(Questions[0])) - 1); q++) {
  if (CurAns [q] != 0) {
   if (TrainingDat[c].q[q] == 0)
    Characters[c].info.q[q] = CurAns [q];
   else
   if (TrainingDat[c].q[q] != CurAns [q])
    Characters[c].info.q[q] = 0;
  }
 }
}
void insertchar (char* name) {
 for (unsigned int c = 0; c != CharactersLen; c++) {
  if (!strcmp (Characters[c].info.name, name)) {
   trainchar (c);
   return;
  }
 }
 for (unsigned char i = 0; i != 20 +1; i++)
...
begin:
 ...
 for (unsigned char i = 0; i != 20; i++) {
  unsigned int qu = getquestion ();
  printf ("%u\t%s (Y/N)\t", i+1, Questions[qu]);
  unsigned char an = parseans (getchar ());
  while (getchar () != '\n');
  train (qu, an);
 }
 ...
 if (parseans (getchar ()) == 2) {
  ...
  char name [20 +2];
  char fmt [20]; snprintf (fmt, (sizeof(fmt) / sizeof(fmt[0])), "%%%u[^\n]", (unsigned int)(20 +1));
  while (getchar() != '\n');
  do printf ("Aww...\nWho are you, then? ");
